...
I found that this algorithm to be MUCH faster than looking through all the values, but still dropped the relevant dud columns quite accurately, allowing me to build a model with 99.71% accuracy.

The other part of my heuristic was to compare the column names in the training set and the set for homework submission. I found the column names to be identical in every case except for the classe, X and problem_id columns. I the X and problem_id colums based on this analysis and a little input for the forums.
